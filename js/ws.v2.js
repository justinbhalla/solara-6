// ws.v2.js — WebSocket live sync + awareness
...
export function connectLive(){ const url=liveUrl(); if(!url) return; try{ state.ws=new WebSocket(url); }catch{ return; }
  ...
  state.ws.onmessage=(ev)=>{ let msg={}; try{ msg=JSON.parse(ev.data||'{}'); }catch{} if(msg.type==='hello'){ state.presence.clear(); (msg.online||[]).forEach(p=> state.presence.set(p.id,{name:p.name,email:p.email})); fire('presence:changed'); if(Number(msg.maxseq||0)>state.lastSeq){ fetch(state.apiUrl(`sync?workspace_id=${encodeURIComponent(state.WORKSPACE_ID)}&since=${state.lastSeq}`), { headers:{ 'Authorization':`Bearer ${state.sessionJWT}` }}).then(r=>r.ok?r.json():Promise.reject()).then(applySyncResponse).catch(()=>{}); } return; } if(msg.type==='presence/join'){ if(msg.by && msg.by!==state.currentUserKey){ state.presence.set(msg.by,{name:msg.name||'',email:msg.email||''}); fire('presence:changed'); } return; } if(msg.type==='presence/leave'){ state.presence.delete(msg.by); fire('presence:changed'); return; }
    ...
    if(msg.type==='op' && msg.ws===state.WORKSPACE_ID){ const { seq, op }=msg; if(seq!==state.lastSeq+1){ fetch(state.apiUrl(`sync?workspace_id=${encodeURIComponent(state.WORKSPACE_ID)}&since=${state.lastSeq}`), { headers:{ 'Authorization':`Bearer ${state.sessionJWT}` }}).then(r=>r.ok?r.json():Promise.reject()).then(applySyncResponse).catch(()=>{}); return; } if(state.sentButUnacked.has(op.op_id)){ state.sentButUnacked.delete(op.op_id); state.lastSeq=seq; stampSync(); return; } import('./oplog.v2.js').then(m=> m.applyOpLocal(op, true)); state.lastSeq=seq; stampSync(); return; } };
}
